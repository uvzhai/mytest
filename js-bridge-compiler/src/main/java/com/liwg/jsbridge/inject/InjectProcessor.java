...
public class InjectProcessor extends AbstractProcessor {
    ...
    private void processImpl(RoundEnvironment roundEnv) throws ClassNotFoundException, IOException {
        ...
        objectSb.append("if(window.EasyJS&&window.EasyJS.injectFlag==0){var readyEvent;try{readyEvent=new Event('LctJSBridgeReady');}catch(e){ readyEvent = doc.createEvent('Event');readyEvent.initEvent('LctJSBridgeReady');}document.dispatchEvent(readyEvent);window.EasyJS.injectFlag=1}");
